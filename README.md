# Aletheia — Algorithmic Forex Trading Engine
...
> **On the name & scope:** This began as an ambition to build a high-frequency trading system. I made a deliberate engineering decision to start with a *correct, fully-tested, observable* foundation — a swing/intraday engine operating on 5-minute candles — rather than chase latency before the fundamentals were sound. Lower-timeframe execution and latency optimisation are on the roadmap; a trustworthy strategy engine and an honest backtest came first. Getting those right is the hard part, and the prerequisite for anything faster.

> **Status:** Core engine, backtesting, and live execution are complete and running against the OANDA practice account, including position reconciliation and Prometheus/Grafana monitoring. Currently in extended paper-trading validation. See [Project Status & Roadmap](#project-status--roadmap).
...
- [The Engineering I'm Proudest Of: Trusting the Backtest](#the-engineering-im-proudest-of-trusting-the-backtest)
...
## The Engineering I'm Proudest Of: Trusting the Backtest

The hardest and most valuable work on this project wasn't a feature — it was catching my own mistake.

A backtest is only useful if it runs the *same logic* as the live engine. Mine didn't. When I audited the two paths, I found that SMT divergence — one of the confirmation signals — was being computed **once for an entire backtest run and then frozen**, rather than recalculated candle-by-candle the way the live engine does it. That quietly leaked future information into past decisions: a textbook **look-ahead bias**. My results looked strong partly because they were cheating.

I rebuilt the backtest to compute SMT **per candle** from only the data visible at each moment (`runWithLiveSmt`), mirroring `LiveSignalService.detectSmt()` exactly. The numbers became more modest — and finally trustworthy.

This is why the codebase now treats backtest/live fidelity as a first-class, structurally-enforced property (see [Design Principles](#design-principles)): every trade-management rule that exists live has a backtest mirror, no-look-ahead is baked into how candle windows are built, and the same `SignalAggregator` runs in both paths. **A backtest you can't trust is worse than no backtest — it's confident and wrong.**

The same discipline drove a three-year, four-pair backtest that honestly surfaced a pair (AUD/USD) which looked excellent in its first year and then degraded to a loss over the following two — exactly the kind of "lucky year vs. real edge" distinction that multi-year testing exists to expose.
...
- Redis — *provisioned in the dev stack but not yet wired in. Planned as a persistent cache for the economic calendar (currently cached in-memory). See [Roadmap](#project-status--roadmap) (M10).*
- Prometheus + Grafana — metrics and a pre-built dashboard (`grafana/provisioning/dashboards/aletheia-dashboard.json`), provisioned from version-controlled config
...
- Metrics are exposed at `/actuator/prometheus` (Spring Boot Actuator), scraped by Prometheus every 15 seconds, and visualised in a pre-provisioned Grafana dashboard (`grafana/provisioning/dashboards/aletheia-dashboard.json`) at `localhost:3000` — the dashboard and its Prometheus datasource are both provisioned from version-controlled config, so the whole observability stack reproduces from a fresh `docker compose up`.
...
| `trading.calendar.csv-path` | `data/calendar_current.csv` | Calendar-service fallback CSV. The loader auto-detects both the `scheduled_time_utc,name,currency,impact,source` and `date,time,currency,impact,event` schemas. |
...
See each module's README for the config keys it specifically consumes.
...
3. **No look-ahead, enforced structurally.** Both `BacktestEngine.runInternal`/`runWithLiveSmt` build every candle window with an explicit `!c.time().isAfter(now)` filter — it's not a convention that could quietly slip, it's baked into how the visible-candle lists are constructed each iteration. This principle is why the SMT look-ahead bug (see [Trusting the Backtest](#the-engineering-im-proudest-of-trusting-the-backtest)) can't recur.
...
| **M4** | Economic calendar / news guard, with HTTP service + CSV fallback (dual-schema) | ✅ Complete |
...
| **M10** | Redis-backed persistent calendar cache (replacing in-memory), so news-guard state survives restarts | 📋 Planned |
| **M11** | Lower-timeframe / reduced-latency execution path (the original HFT ambition), once the current model is validated | 📋 Planned |
...
- `SimulatedTrade`/`BacktestEngine` rewritten to mirror the exact live two-stage TP1/TP2 model, with per-candle (not precomputed) SMT divergence — closing the look-ahead-bias gap described in [Trusting the Backtest](#the-engineering-im-proudest-of-trusting-the-backtest).
...
- `CsvCalendarLoader` made schema-tolerant — it now auto-detects and parses both the calendar service's UTC-timestamp format and the hand-authored EST date/time format.
...
- **`DukascopyHistoryLoader` has no on-disk cache of its own** — `aletheia-backtest`'s `BacktestRunner` adds a disk-cache layer *around* it (keyed by instrument/date-range CSVs under `data/cache/`), while the live `DxyFeedService` maintains its *own* separate warm-start cache. A direct call to the loader with no surrounding cache re-downloads every time.
...
- **Live trade management is built and running, but still in validation.** The full TP1 → breakeven → TP2 lifecycle is implemented and reconciles against real broker state, but the extended paper-trading run (M9) is how it earns confidence across many real trades over time. It should be described as "complete and forward-testing," not "battle-tested in production."
