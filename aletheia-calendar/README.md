...
## `CsvCalendarLoader` — CSV fallback source (dual-schema)

Implements `CalendarDataSource`. `fetch()` reads every line, skips the header, filters to `[from, to]` inclusive (bounds converted via `America/New_York`, with `to.plusDays(1)` as the exclusive upper edge). Any line that fails to parse is skipped individually and logged (`[CsvCalendarLoader] Skipping line ...`) — not fatal to the whole load, mirroring the HTML parser's row-level fault tolerance.

**The loader auto-detects which of two CSV schemas a file uses**, from its header row, so both the format the calendar service emits and the hand-authored historical format load through the same class:

**Schema A — combined UTC timestamp** (what the calendar service / `data/calendar_current.csv` uses):
...
The timestamp is parsed directly as an absolute `Instant` (already UTC); the event name is column 2.

**Schema B — split date + EST-local time** (the hand-authored historical files, `data/calendar_2023.csv`, `calendar_2024.csv`, etc., and the test fixtures):
...
- `date`: `yyyy-MM-dd`; `time`: `HH:mm`, both interpreted in **America/New_York** (EST/EDT), then converted to an absolute `Instant`.
- split with `line.split(",", 5)` so the event-name field (column 5) may itself contain commas.

Schema detection: the header is inspected once — if it contains `scheduled_time_utc`, Schema A is used; otherwise the loader defaults to Schema B (preserving the original behaviour for any header-less or unrecognised file). Both schemas resolve to the same absolute `Instant` internally, so downstream code never knows or cares which format the file was in.

Impact mapping is shared across both schemas: `HIGH`/`RED` → `HIGH`; `MEDIUM`/`MED`/`ORANGE` → `MEDIUM`; anything else (including `LOW`/`YELLOW`) → `LOW`.

> This dual-schema tolerance closed a real earlier bug: `aletheia-api` defaults its fallback CSV path to `data/calendar_current.csv`, which is in Schema A, while the loader originally only understood Schema B — so every row in that specific file was silently skipped if the CSV fallback was ever exercised. The loader now parses both, and `CsvCalendarLoaderTest` covers the Schema-B path directly.
